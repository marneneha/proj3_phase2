...
def action(node, parent_node_g_cost, canvas, rpm1, rpm2, R, L):
    ...
    velocity_list.append((0.5*R*(rpm2+rpm1)*np.cos(np.deg2rad(theta_initial)), -0.5*R*(rpm2+rpm1)*np.sin(np.deg2rad(theta_initial)), (-R/L)*(rpm1 - rpm2)))
    ...
    # print(canvas)
    # cv2_imshow(canvas)
    return True, new_node, g_cost, False, velocity_list
...
def astar(start_position, final_position, canvas, rpm1, rpm2, R, L):
  # TO FILL
    ...
    while(len(open_list)):
        ...
        print("ACTIONNNNN",action(parent_node, parent_node_g_cost, canvas, 0, rpm1, R, L))
        for obstacle_check, node, g_cost, visibility_check, velocity_list in [
