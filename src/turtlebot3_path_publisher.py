...
rospy.init_node('turtlebot3_path_publisher_node', anonymous=True)
rate = rospy.Rate(1)
twist = Twist()
...
def action(node, parent_node_g_cost, canvas, rpm1, rpm2, R, L):
    ...
    if not (0 <= x < canvas.shape[1] and 0 <= y < canvas.shape[0] and not obstacle_checkpoint(x, y, canvas)):
        return False, node, parent_node_g_cost, False, action_set
# theta is in degree
    new_node = (x, y, theta)
    # print(canvas)
    # cv2_imshow(canvas)
...
def astar(start_position, final_position, canvas, rpm1, rpm2, R, L):
  # TO FILL
    ...
    action_set = [0,0]
    hq.heappush(open_list, [f_cost, h_cost, g_cost, tuple(start_position), tuple(start_position), tuple(action_set)])
    ...
    while(len(open_list)):
        ...
        for obstacle_check, node, g_cost, visibility_check, action_set in [
        ...
        action(parent_node, parent_node_g_cost, canvas, rpm2, rpm1, R, L)
            ...
            if obstacle_check:
                ...
                # if it is not in OL update the OL                 
                    ...
                    f_cost = h_cost+g_cost
                    # print("f_cost", f_cost)
                    new_open_list_element = [f_cost, h_cost, g_cost, node, parent_node, action_set]
